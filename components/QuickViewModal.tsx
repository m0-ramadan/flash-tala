...
export default function QuickViewModal({
  ...
  const inStock = product?.meta?.in_stock ?? (product?.stock ?? 0) > 0;
                      ...
                      {[product?.image, ...gallery.map((g) => g.path)]
                        ...
                        .map((src, idx) => (
                          <button
                            ...
                            className={`h-16 w-16 flex-none rounded-xl overflow-hidden ring-2 transition ${
                              String(src) === activeImg
                                ? "ring-pro"
                ...
                <div className="p-4 md:p-6 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="text-xs text-gray-500">
                      التقييم: {Number(product?.average_rating || 0).toFixed(1)}{" "}
                      / 5
                      {typeof product?.total_reviews === "number" && (
                        <span className="ms-2 text-gray-400">
                          ({product.total_reviews} مراجعة)
                        </span>
                      )}
                    </div>
