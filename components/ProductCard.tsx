...
export default function ProductCard({
  ...
  const handleAddToCart = async () => {
    ...
    await addToCart(id, {
      ...
      print_locations: selectedPrintLocations.length
        ? selectedPrintLocations
        : [],
      embroider_locations: selectedEmbroiderLocations.length
        ? selectedEmbroiderLocations
        : [],
        ...
        <div className={`relative w-full h-[150px] md:h-[240px] bg-gray-50`}>
          ...
          <div className="absolute top-1 md:top-3 inset-x-1  md:inset-x-3 flex items-center justify-between z-10">
            ...
            <div className="flex flex-col gap-1 items-start">
              {product?.price_text && (
                <span className="md:px-3 p-1 truncate max-w-[80px] md:max-w-[140px] text-[8px] md:text-[11px] font-extrabold rounded-lg md:rounded-full bg-emerald-500 text-white ring-1 ring-black/5 shadow-sm">
          ...
          >
            <div
              className={`w-9 h-9 rounded-full flex items-center justify-center shadow-lg ring-1 ring-black/5 ${
                inStock
                  ? "bg-pro text-white"
