...
export function TotalOrder({
  ...
  const totalAfterCoupon =
    couponNewTotal ?? Math.max(0, total - couponDiscount);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between text-sm">
        <p className="font-semibold text-slate-700">
          المجموع ({itemsCount} عناصر)
        </p>
        <p className="font-bold text-slate-900">
          {money(subtotal)} <span className="text-xs">ج.م</span>
        </p>
      </div>
      ...
      {couponDiscount > 0 && (
        <div className="flex items-center justify-between text-sm">
          ...
          <p className="font-extrabold text-emerald-600">
            - {money(couponDiscount)} <span className="text-xs">ج.م</span>
          </p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <p className="font-semibold text-slate-900">
          {money(totalAfterCoupon)} <span className="text-xs">ج.م</span>
        </p>
      </div>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <p className="font-semibold text-slate-900">
          {money(totalWithoutTax)} <span className="text-xs">ج.م</span>
        </p>
      </div>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <p className="font-semibold text-slate-900">
          {money(taxAmount)} <span className="text-xs">ج.م</span>
        </p>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 mt-2 border-t border-slate-200">
        ...
        <p className="text-lg font-extrabold text-[#14213d]">
          {money(totalWithShipping)} <span className="text-sm">ج.م</span>
        </p>
      </div>
    </div>
  );
