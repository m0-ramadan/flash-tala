...
export const StickerForm = forwardRef<StickerFormHandle, StickerFormProps>(
  function StickerForm(
    ...
    const [designSendMethod, setDesignSendMethod] =
      useState<DesignSendMethod>(null);
    ...
    const validateCurrentOptions = useCallback(() => {
      ...
      if (
        needPrintLocation &&
        (!Array.isArray(printLocations) || printLocations.length === 0)
      )
        isValid = false;
    ...
    const loadSavedOptions = useCallback(async () => {
      ...
      try {
        ...
        const sizeFrom = extractValueFromOptions(
          saved.selected_options,
          "المقاس",
        );
        const colorFrom = extractValueFromOptions(
          saved.selected_options,
          "اللون",
        );
        const matFrom = extractValueFromOptions(
          saved.selected_options,
          "الخامة",
        );
        const pmFrom = extractValueFromOptions(
          saved.selected_options,
          "طريقة الطباعة",
        );

        const qtyFrom = extractValueFromOptions(
          saved.selected_options,
          "كمية المقاس",
        );
        const totalFrom = extractValueFromOptions(
          saved.selected_options,
          "سعر المقاس الإجمالي",
        );
        const unitFrom = extractValueFromOptions(
          saved.selected_options,
          "سعر الوحدة",
        );
        const locsFrom = extractValuesFromOptions(
          saved.selected_options,
          "مكان الطباعة",
        );
        ...
        if (q && apiData?.sizes) {
          const sz = apiData.sizes.find(
            (s: any) => s?.name === (sizeFrom || saved.size),
          );
          const tier =
            (sz?.tiers || []).find((x: any) => Number(x?.quantity) === q) ||
            null;
    ...
    const saveAllOptions = async () => {
      ...
      const payload: any = {
        ...
        selected_options,
        quantity:
          needSizeTier && opts.size_quantity
            ? Number(opts.size_quantity)
            : undefined,
      };
    ...
    const designServiceValue = String(
      optionGroups?.["خدمة تصميم"] ?? "اختر",
    ).trim();
              ...
              >
                ...
                >
                  ...
                  {sizeTiers.map((t: any) => {
                    ...
                    const showTotal =
                      backendTotal > 0 ? backendTotal : computed;
                        ...
                        <div className="mt-4">
                          ...
                          <div className="flex flex-col gap-3">
                            ...
                            <p className="text-xs text-slate-500 font-bold">
                              سيتم إرسال الملف مع تفاصيل المنتج والخيارات إلى:{" "}
                              {API_URL}/file
                            </p>
                          </div>
                        </div>
...
export function buildIdsPayload(apiData: any, opts: SelectedOptions) {
  ...
  const materialObj = apiData?.materials?.find(
    (m: any) => m?.name === opts.material,
  );
  const pmObj = apiData?.printing_methods?.find(
    (p: any) => p?.name === opts.printing_method,
  );
  ...
  const printLocationIds =
    Array.isArray(opts.print_locations) && opts.print_locations.length
      ? opts.print_locations
          .map(
            (name: any) =>
              apiData?.print_locations?.find((pl: any) => pl?.name === name)
                ?.id,
          )
...
export function buildSelectedOptionsWithPrice(
  apiData: any,
  opts: SelectedOptions,
) {
  const selected_options: Array<{
    option_name: string;
    option_value: string;
  ...
  if (opts.color && opts.color !== "اختر") {
    ...
    selected_options.push({
      ...
      additional_price: num(c?.additional_price),
    });
  }
  ...
  if (opts.material && opts.material !== "اختر") {
    ...
    selected_options.push({
      ...
      additional_price: num(m?.additional_price),
    });
  }
  ...
  if (opts.printing_method && opts.printing_method !== "اختر") {
    const pm = apiData?.printing_methods?.find(
      (x: any) => x.name === opts.printing_method,
    );
    selected_options.push({
