  ...
  return (
    <div className="my-4 gap-2 flex flex-col">
      ...
      <div className="flex text-sm items-center justify-between text-black">
        ...
        {/* <p className="text-md">
                    {money(subtotal)} <span className="text-sm ms-1">ج.م</span>
                </p> */}
      </div>
      ...
      {hasCoupon && (
        <div className="flex items-center justify-between text-sm">
          ...
          <p className="font-extrabold text-emerald-700">
            ...
            <span className="text-sm ms-1">ج.م</span>
          </p>
        </div>
      )}
      ...
      {/* <div className="flex items-center justify-between text-sm">
                ...
                {isShippingFree ? (
                    ...
                    <p className="font-semibold">
                        {money(shippingFee)} <span className="text-sm ms-1">ج.م</span>
                    </p>
                )}
            </div> */}
...
export default function PaymentPage() {
  const { cart, cartCount, removeFromCart, updateQuantity, subtotal, total } =
    useCart();
  ...
  const [checkoutSummary, setCheckoutSummary] =
    useState<CheckoutSummaryV1 | null>(null);
  ...
  const handleCompletePurchase = async () => {
    ...
    if (!checkoutSummary) {
      Swal.fire(
        "تنبيه",
        "لا توجد بيانات ملخص الطلب. يرجى الرجوع للسلة ثم المحاولة مرة أخرى.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      const orderData: any = {
        ...
        customer_name: (selectedAddress as any)?.full_name
          ? String((selectedAddress as any).full_name)
          : "",
        customer_phone: (selectedAddress as any)?.phone
          ? String((selectedAddress as any).phone)
          : "",
        customer_email: (selectedAddress as any)?.email
          ? String((selectedAddress as any).email)
          : "",
      ...
      if (paymentMethod == "1") {
        ...
        setRedirectMessage(
          result?.data?.message || "جاري توجيهك إلى بوابة الدفع...",
        );
        setRedirecting(true);
        console.log(result);
        setTimeout(() => {
  ...
  return (
    <div className="container pb-10 pt-6">
      ...
      <div className="flex items-center gap-2 text-sm mb-4">
        <button
          onClick={() => router.back()}
          className="cursor-pointer text-pro-max font-bold flex items-center gap-1"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="col-span-1 lg:col-span-2 space-y-4">
            ...
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  اختر العنوان المناسب أو أضف عنوان جديد.
                </p>
              </div>
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    <p className="text-slate-700 font-extrabold">
                      ...
                      <span className="text-slate-900">
                        {selectedAddress
                          ? `${(selectedAddress as any).city} - ${(selectedAddress as any).area}`
                          : "لم يتم اختيار عنوان"}
                      </span>
                    </p>
                    ...
                    {selectedAddress && (
                      <div className="mt-2 text-sm text-slate-600 space-y-1">
                        ...
                        <p className="font-semibold">
                          {(selectedAddress as any).full_name}{" "}
                            ...
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <span
                                className={`text-xs font-extrabold rounded-full px-3 py-1 border ${
                                  active
                                    ? "bg-white border-pro-max text-pro-max"
                    ...
                    {loadingShipping ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {shippingOptions.map((option) => {
                          const active =
                            selectedShipping?.deliveryOptionId ===
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="text-right">
                                  ...
                                  <p className="text-[10px] text-slate-500 font-semibold">
                                    {option.avgDeliveryTime || "توصيل سريع"}
                                  </p>
                                </div>
                              </div>
                      ...
                      <div className="p-4 bg-slate-50 rounded-3xl border border-dashed border-slate-200 text-center">
                        <p className="text-sm text-slate-500 font-semibold">
                          لا توجد خيارات شحن متاحة لهذا العنوان حالياً
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-200">
              ...
              <p className="text-sm text-slate-500 mt-1">
                اختر الطريقة الأنسب لإتمام الطلب.
              </p>
            </div>
            <div className="p-5">
              <BankPayment
        ...
        <div className="col-span-1 space-y-4 lg:sticky lg:top-[150px] h-fit">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-5">
            ...
            {/* ملخص الطلب */}
            <div className="mt-4">
              <h4 className="text-md font-extrabold text-pro mb-3">
                ملخص الطلب
              </h4>
              <TotalOrder
            ...
            <div className="mt-4">
              <Button
                ...
                sx={{
                  ...
                  "&:hover": {
                    backgroundColor: loading ? "#9ca3af" : "#0f1a31",
                  },
                  color: "#fff",
                  gap: "10px",
                  px: "20px",
              ...
              {!checkoutSummary && (
                <p className="text-amber-700 text-center mt-2 text-sm font-semibold">
                  ملخص الطلب غير موجود
                </p>
              )}
            </div>
          </div>
