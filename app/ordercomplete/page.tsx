...
function computeOrderItemPricing(orderItem: AnyObj) {
  ...
  const base = product?.has_discount
  ...
  const selectedSize = options.find((o: AnyObj) =>
    String(o.option_name || "").includes("المقاس"),
  )?.option_value;

  if (selectedSize && sizes.length) {
    const sizeObj = sizes.find(
      (s: AnyObj) => String(s.name).trim() === String(selectedSize).trim(),
    );
  ...
  const printingMethods = Array.isArray(product?.printing_methods)
    ? product.printing_methods
    : [];
  const printLocations = Array.isArray(product?.print_locations)
  ...
  for (const opt of options) {
    ...
    if (name === "طريقة الطباعة") {
      const pm = printingMethods.find(
        (x: AnyObj) => String(x.name).trim() === value,
      );
...
function SummaryBlock({
  ...
  orderData?: AnyObj | null;
}) {
  // Prioritize API data if available
  const subtotal = orderData ? n(orderData.subtotal) : n(summary?.subtotal);
  const shippingFee = orderData
    ? n(orderData.shipping_amount)
    : n(summary?.shipping_fee);
  const taxAmount = orderData
    ? n(orderData.tax_amount)
    : n(summary?.tax_amount);
  const discountAmount = orderData
    ? n(orderData.discount_amount)
    : n(summary?.coupon_discount);
  const totalItems = orderData
    ? Array.isArray(orderData.items)
      ? orderData.items.length
      : 0
    : n(summary?.items_length);
...
export default function OrderCompletePage() {
  ...
  const computed = useMemo(() => {
    ...
    const subtotal = computedItems.reduce(
      (acc: number, it: AnyObj) => acc + n(it._line),
      0,
    );
    ...
    <div className="container py-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {loading ? (
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              ...
              <p className="text-slate-600 mt-2">
                تأكد من وجود orderId في الرابط وأنك مسجل دخول.
              </p>
            ...
            <>
                ...
                <div className="flex-col flex items-start md:flex-row justify-between gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-extrabold whitespace-nowrap text-sm text-slate-900">
                        {order.order_number}
                      </span>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h5 className="font-extrabold text-xl text-slate-900 mb-3">
                      عنوان الشحن
                    </h5>
                        ...
                        <div className="text-right">
                          ...
                          {order.oto_order_id && (
                            <p className="text-[10px] text-slate-500 font-semibold">
                              رقم طلب الشحن: {order.oto_order_id}
                            </p>
                          )}
                        </div>
                      </div>
        ...
        <div className="col-span-1 space-y-4 lg:sticky lg:top-[140px] h-fit">
          {/* Items */}
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6">
            <h2 className="font-extrabold text-xl text-slate-900 mb-4">
              تفاصيل الطلب
            </h2>
